...
### Step 3: Free Memory for LLMs (Headless Mode)
- Once the system is booted and connected, disable the graphical desktop (Gnome) to free up ~1.5 GB of unified system RAM for running LLMs (Ollama).
- Command to disable: `sudo systemctl set-default multi-user.target`
- Command to restore if needed: `sudo systemctl set-default graphical.target`
